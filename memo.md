...
- `base` のtable 使ったパターンが、a-shell では2度目実行ができず調査していた
- 素振りとして、`UIActivityIndicatorView` のをやっていた
  - `style` 定義が間違えていた
  - 違う原因かと思い、メモリの状態確認や、window やscene の方調べたりしちゃった
    - `animation` のやつは、ほかの`View` の指示もあるから、混じってるっぽいね


## `enumerations.py` の`dataclass` 問題。。。

Python の新し目のモジュールとして、便利そう！と気軽に導入したけども
他のコードを見てみると、普通にclass でやっているっぽい？

基本をclass して`enum` を使うかー。くらいでいいのかしら？


## とりあえず、コード綺麗に

- base としているものを先に整える
- base 終えたら、もうmain にマージするかー



# 📝 2025/01/23

## 基本的な`viewController` のコードの書き方

- View ライフサイクル
  - class の中のどの位置に書くか
  - そもそも全体的に書くことが必要か
    - `import` が減って見やすくなることもあり？
    - オーバーライドするので、しないなら書かなくてよくね？
  - 実装面でコード数が多いと、煩雑になる

- とはいえ、Apple のサンプルコードに寄せた方が良いよね。。。
...
- キャレット
  - サンプルも文字とキャレット位置がめちゃくちゃ
  - XCode で古いバージョンで確認すればどうにかなる？
- `symbol` の色が変わらない
  - Dark Mode 時でも、黒文字となっている？
  - どの属性に色をつける？
    - サンプルコードとして、直接指定はしていないので、こっちもその方針でいきたい


# 📝 2024/12/18

readme に、進捗状況を記載するお気持ち

サンプルのコードと、実装との対比。実装時のメモやペンディングしている状況についてもメモをしていきたい。



markdown でのtable レイアウトは、携帯閲覧だと地獄。他の方法で良い感じにまとめたい。
スプレッドシートと連動させて、吐き出したりしてみる？
